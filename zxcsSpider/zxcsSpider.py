    ...
    if len(pageNumList)==0:
        ...
        if pagenum>1:
            for index in range(1,pagenum+1):
                ...
                links=get_one_page_links(r'http://www.zxcs.me/tag/'+tag+'/page/'+str(index))
...
def loadErrorLog():
    with open('log/201905061153.log','r',encoding='utf8') as f:
        logstr=f.read()
        ...
        for link in bookurlList:
            tag=get_book_tag(link)
            bookinfo=get_one_bookInfo(link,tag)
            ps1path = tag + '.ps1'
            ps1append = ''
            newline='\naria2c "-s2 %s" "%s" '%(bookinfo['link1'],bookinfo['link2'])
    ...
    for tag in tagList:
        ...
        for link in linklist:
            ...
            try:
                ...
                one_download_str= 'aria2c "-s2 %s" "%s" '%(bookinfo['link1'],bookinfo['link2'])
                ps1Str+='\n'+one_download_str
